...
do
  ...
  do
    ...
    for dataset in albert bank blood calhousing compas covertype credit_card_default creditg diabetes electricity eye_movements heart income jungle road_safety
    do
      # Zero-shot
      # eval_before_training=True
      # num_steps=0
      ...
      for seed in 0
      do
        CUDA_VISIBLE_DEVICES=${cuda_device} CONFIG_PATH=/hdd/hans/t-few/configs HF_HOME=/hdd/hans/huggingface \
        python -m src.pl_synthetic -c ${model}.json+ia3.json+global.json -k dataset=${dataset} load_weight="pretrained_checkpoints/${model}_ia3_finish.pt" num_steps=${num_steps} num_shot=${num_shot} \
